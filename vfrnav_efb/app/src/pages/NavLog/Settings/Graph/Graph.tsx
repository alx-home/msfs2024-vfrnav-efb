...
export const Graph = ({ datasets: baseDatasets, datasetLegend, xUnit, yUnit, xMargin, datasetUnit, xLegend, yLegend,
   ...
   const [resize, setResizeRef] = useResize()

   const [graphWidth, graphHeight] = useMemo(() => [(resize?.width ?? 0), (resize?.height ?? 0)], [resize])
   const [divs, step] = useMemo(() => {
      const base = [Math.ceil((bounds[0][1] - bounds[0][0]) / baseStep[0]), Math.ceil((bounds[1][1] - bounds[1][0]) / baseStep[1])]
      const divs = [...base]
      const step = [...baseStep]

      for (let i = 2; (graphWidth / divs[0] < 40) && (divs[0] > 1); ++i) {
      ...
      for (let i = 1; (graphHeight / divs[1] < 40) && (divs[1] > 1); ++i) {
      ...
      return [divs, step]
   }, [bounds, baseStep, graphWidth, graphHeight])
   ...
   return <div className="flex flex-col w-full h-full mb-12 pr-10 pl-10 mt-2 select-none" inert={!(active ?? true)}>
      <div className="flex flex-col w-full h-full" ref={e => { setResizeRef(e); }}>
            ...
            <div className="relative flex w-full h-full">
               <div className="relative flex flex-col w-full h-full"
                  ...
                  <Content active={active} baseDatasets={baseDatasets} bounds={bounds} fullCoverMode={fullCoverMode}
                     setBaseDatasets={setBaseDatasets} setBaseDatasetsWeak={setBaseDatasetsWeak}
                     showLegend={showLegend} span={span} datasetLegend={datasetLegend} datasetStr={datasetStr}
                     xValuesStr={xValuesStr} yValuesStr={yValuesStr} xMargin={xMargin}
         ...
         <div className="flex flex-row w-full">
            ...
            <AxisX bounds={bounds} graphWidth={graphWidth} lastRatio={lastRatio} xValues={xValues} xValuesStr={xValuesStr} />
         </div>
      </div>
   </div>
}
