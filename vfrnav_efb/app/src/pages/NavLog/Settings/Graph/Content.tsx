...
export const Content = ({ baseDatasets, bounds, span, showLegend, datasetLegend, datasetStr,
   ...
   setBaseDatasets, setBaseDatasetsWeak, xMargin
}: {
   active?: boolean,
   showLegend?: boolean,
   ...
   const addPoint = useCallback((dataset_index: number, index: number) => {
      const cbounds = graphRef.current!.getBoundingClientRect();
      const pos = [mouse!.x - cbounds.left, cbounds.height - (mouse!.y - cbounds.top)];
   ...
   useEffect(() => {
      if (mouseUp && editIndex !== undefined) {
         setEditIndex(undefined)
         setBaseDatasets(values => values.toSpliced(editIndex[0], 1, values[editIndex[0]].toSpliced(editIndex[1], editMode ? 1 : 0, [...getBoundedPointFromPixels(editIndex[0], editIndex[1], [mouse!.x, mouse!.y]), false])))
         setEditMode(false)
      }
   }, [editIndex, editMode, getBoundedPointFromPixels, mouse, mouseUp, setBaseDatasets])


   return <div className="absolute flex w-full h-full">
      <div className="relative flex w-full h-full" ref={graphRef}>
         {datasets.map((dataset, dataset_index) => dataset.filter((_, index) => index !== 0).map((elem, index) =>
            <Line key={index} index={index} parentRef={graphRef}
               ...
               dataset_index={dataset_index} coords={[dataset[index], elem]} bounds={bounds} />
         ))}
