...
import { Deviation } from "./Settings/Deviation";
import { Fuel } from "./Settings/Fuel";
...
export const Settings = ({ currentTab: currentPage }: {
   currentTab: string
}) => {
   const [tab, setTab] = useState<SettingsTabs>('Deviation')
   const tabElems = useMemo(() => [
      <Deviation key="deviation" curentPage={tab} />,
      <Fuel key="fuel" curentPage={tab} />
   ], [tab]);
