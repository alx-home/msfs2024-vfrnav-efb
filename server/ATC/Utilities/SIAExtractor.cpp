...
Extractor::GetFrequencies(std::string const& icao) const {
   return [this, icao] -> Promise<std::vector<Frequency>, false> {
     ...
     [this] -> Promise<Frequencies, false> {
        ...
        for (auto const& icao : icaos) {
           ...
           co_await RetrieveHoldingPoint(icao).Catch([icao](std::exception const& exc) {
              std::cerr << "Failed to retrieve frequencies for " << icao << ": " << exc.what()
                        << std::endl;
           });
        }
...
Extractor::RetrieveUrl() const {
   return [this]() -> Promise<std::string, false> {
...
Extractor::RetrieveIcaos() const {
   return [this]() -> Promise<std::vector<std::string>, false> {
      auto const data = co_await main_.PostHttpRequest(
      ...
      for (size_t pos = icao_start; pos != std::string::npos; pos = data.find("/a", pos),
                  (pos != std::string::npos) && (pos = data.find("AD-2.18", pos + 1))) {

         auto const icao = data.substr(pos - 5, 4);
         if (std::ranges::all_of(icao, [](unsigned char c) { return std::isupper(c); })) {
...
Extractor::RetrieveFrequencies(std::string const& icao) const {
   return [this, icao]() -> Promise<std::vector<Frequency>, false> {
      ...
      for (std::size_t limit = 0; limit < 100; ++limit) {
         ...
         ScopeExit  row_scope{[&] { index = trend + 2; }};
         ...
         for (std::size_t i = 0; i < 5; ++i) {
            ...
            ScopeExit  cell_scope{[&] { index = end + 4; }};
            ...
            for (std::size_t j = 0; index != std::string::npos;
               ...
               if (
                 freq_beg == std::string::npos || freq_end == std::string::npos
                 || freq_end < freq_beg
               ) {
...
Extractor::RetrieveHoldingPoint(std::string const& icao) const {
   return [this, icao]() -> Promise<void, false> {
