...
public:
   ...
   double Update(
     ...
     std::chrono::steady_clock::time_point now = std::chrono::steady_clock::now()
      ...
      auto const output = params_.kp_ * error + params_.ki_ * integral_ + derivate;
...
public:
   ...
   operator double() const { return state_; }

   void Reset(double value) { state_ = value; }
...
WPromise<void>
Aircraft::Wait(std::optional<std::chrono::steady_clock::time_point> timeout) {
   if (timeout) {
      return promise::Race(*update_promise_, main_.Wait(*timeout), WaitDone());
   } else {
      return promise::Race(*update_promise_, WaitDone());
   }
}
...
Aircraft::LandAircraftRollout(Coords<2> const& direction, double slope) {
   return [this, direction, slope]() -> Promise<void> {
      ...
      auto const [init_info, static_info] =
        *(co_await promise::Race(promise::All(*info_, STATIC_INFO), WaitDone()));
      ...
      while (true) {
         auto const info = *(co_await promise::Race(*info_, WaitDone()));
           ...
           promise::All(
             UpdateControl(
               {.pitch_  = target_pitch,
                .speed_  = next_speed,
                .vspeed_ = std::min(
                  0., (std::cos(info.pitch_) * std::sin(info.pitch_) * next_speed - 5) * 1.68781
                )}
             ),
             main_.Wait(100ms)
           ),
...
Aircraft::LandAircraft(Waypoint const& target, double slope, double height) {
   ...
   return [this, target, slope, touch_alt = height] -> Promise<void> {
      ...
      auto const [init_info, static_info] = *(co_await promise::Race(
        promise::All(sim_connect.AIReleaseControl(id.dwObjectID), *info_, STATIC_INFO), WaitDone()
      ));
      ...
      while (true) {
         ...
         auto const pitch_guidance = [&]() {
            Coords const pos_to_axis{0.0, -deltaAlt};
         ...
         auto const new_pitch_rad = std::clamp(
           pitch_guidance,
           -15.0 * std::numbers::pi / 180.0,
           (flare ? flare_slope : 15.0 * std::numbers::pi / 180.0)
         );

         // Force 0.1kts vspeed at flare to ensure wheels contact the ground
         auto const flare_vspeed =
           flare
             ? std::min(
                 0., std::cos(new_pitch_rad) * (std::sin(new_pitch_rad) * info.true_airspeed_ - 0.1)
               )
             : 0;
         ...
         if (now - last_log > 100ms) {
            ...
            auto const log_field =
              [&](std::string_view label, auto value, std::string_view suffix = "") {
                 ...
                 std::cout << std::left << std::setfill(' ') << std::setw(w_field) << field.str();
              };
            ...
            log_section("pitch", [&] {
               ...
               log_field("tgt", pitch_guidance * 180.0 / std::numbers::pi, "deg");
            });
         ...
         co_await promise::Race(
           promise::All(
             UpdateControl({
               .pitch_   = new_pitch_rad,
               .heading_ = heading_guidance * 180.0 / std::numbers::pi,
...
WPromise<void>
Aircraft::UpdateControl(Aircraft::Control&& control) {
   return [this, control = std::move(control)]() -> Promise<void> {
      auto const lock = std::move(co_await promise::LockGuard::Create(control_mutex_));
...
Aircraft::AlignWithWaypoint(Waypoint const& current_wp, smc::TrafficInfo const& info) {
   return [this, &current_wp, &info] -> Promise<void> {
      ...
      while (true) {
         auto const current_info = *(co_await promise::Race(*info_, WaitDone()));
...
WPromise<void>
Aircraft::AircraftControlLoop() {
   return [this] -> Promise<void> {
      ScopeExit _{[] { std::cout << "Aircraft[control_loop]: loop ended" << std::endl; }};
      ...
      LowPassFilter pitch_filter{{
        ...
        .alpha_     = 0.2,
        .delta_min_ = -0.1 * std::numbers::pi / 180.0,
        .delta_max_ = 0.1 * std::numbers::pi / 180.0,
      }};

      Control last_control{};
      auto&   sim_connect = main_.SimConnect();

      std::optional<steady_clock::time_point> next_update = std::nullopt;
      steady_clock::time_point                last_update = steady_clock::now();

      steady_clock::duration const refresh_rate{25ms};
      steady_clock::duration const slow_refresh_rate{100ms};

      auto refresh_counter = 0;

      promise::LockGuard lock{control_mutex_, [this](promise::Mutex& mutex) noexcept {
                                 main_.MainPool::Dispatch([&mutex] { mutex.UnLock(); }).Detach();
                              }};
      co_await lock.Lock();
      Control control{};
      control.Update(control_);

      WPromise<void> update_promise = control_cv_.Wait(lock);

      while (true) {
         std::optional<steady_clock::duration> wanted_refresh = std::nullopt;

         co_await promise::Race(update_promise, Wait(next_update));
         if (update_promise.Done()) {
            // Update was triggered by a control change.
            assert(lock.OwnsLock());

            control.Update(control_);

            update_promise = control_cv_.Wait(lock);

            if ((steady_clock::now() - last_update) < refresh_rate) {
               next_update = last_update + refresh_rate;
               continue;
            }
         }

         bool const slow_refresh = [&] {
          ...
          * ---------------------- Pitch control ------------------------- *
          * -------------------------------------------------------------- */
         if (control.pitch_.has_value()) {
            auto const current_info = *(co_await promise::Race(*info_, WaitDone()));

            if (!last_control.pitch_.has_value()) {
               pitch_filter.Reset(current_info.pitch_);
            }

            auto const vel_dir = std::atan2(
              current_info.world_vertical_velocity_ * 0.3048,
              std::hypot(current_info.world_lat_velocity_, current_info.world_lon_velocity_)
                * 0.3048
            );
            auto const pitch_error =
              std::remainder(vel_dir - *control.pitch_, 2.0 * std::numbers::pi);

            auto const next_pitch = pitch_filter.Update(pitch_filter + pitch_error);

            if (
              !last_control.pitch_
              || std::fabs(next_pitch - *last_control.pitch_) > 0.08 * std::numbers::pi / 180.0
            ) {
               last_control.pitch_ = next_pitch;
               promises.emplace_back(sim_connect.SetDataOnSimObject(
                 smc::DataId::SET_PITCH_CONTROL, id.dwObjectID, 0, next_pitch
               ));
            }

            if (std::fabs(*control.pitch_ - next_pitch) > 1 * std::numbers::pi / 180.0) {
               wanted_refresh = std::min(wanted_refresh.value_or(refresh_rate), refresh_rate);
            }
         } else {
            last_control.pitch_.reset();
         }

         /* -------------------------------------------------------------- *
          * ---------------------- Heading control ----------------------- *
          * -------------------------------------------------------------- */
         if (control.heading_.has_value()) {
            if (
              !last_control.heading_.has_value()
              || std::fabs(*control.heading_ - *last_control.heading_) > 2  // degrees
            ) {
               wanted_refresh =
                 std::min(wanted_refresh.value_or(slow_refresh_rate), slow_refresh_rate);

               if (slow_refresh) {
                  last_control.heading_ = *control.heading_;
                  promises.emplace_back(sim_connect.SetDataOnSimObject(
                    smc::DataId::SET_AI_HEADING, id.dwObjectID, 0, *control.heading_
                  ));
               }
            }
         } else {
            last_control.heading_.reset();
         }

         /* -------------------------------------------------------------- *
          * ---------------------- Speed control ------------------------- *
          * -------------------------------------------------------------- */
         if (control.speed_.has_value()) {
            if (
              !last_control.speed_.has_value()
              || std::fabs(*control.speed_ - *last_control.speed_) > 5  // kts
            ) {
               wanted_refresh =
                 std::min(wanted_refresh.value_or(slow_refresh_rate), slow_refresh_rate);

               if (slow_refresh) {
                  last_control.speed_ = *control.speed_;
                  promises.emplace_back(sim_connect.SetDataOnSimObject(
                    smc::DataId::SET_AI_SPEED, id.dwObjectID, 0, *control.speed_
                  ));
               }
            }
         } else {
            last_control.speed_.reset();
         }

         /* -------------------------------------------------------------- *
          * ----------------------- VSpeed control ----------------------- *
          * -------------------------------------------------------------- */
         if (control.vspeed_.has_value()) {
            if (
              !last_control.vspeed_.has_value()
              || std::fabs(*control.vspeed_ - *last_control.vspeed_) > 1 * 0.3048  // ft/s
            ) {
               wanted_refresh =
                 std::min(wanted_refresh.value_or(slow_refresh_rate), slow_refresh_rate);

               if (slow_refresh) {
                  last_control.vspeed_ = *control.vspeed_;
                  promises.emplace_back(sim_connect.SetDataOnSimObject(
                    smc::DataId::SET_VSPEED_CONTROL, id.dwObjectID, 0, *control.vspeed_
                  ));
               }
            }
         } else {
            last_control.vspeed_.reset();
         }

         if (wanted_refresh.has_value()) {
            next_update = steady_clock::now() + wanted_refresh.value();
         } else {
            last_control = {};
            next_update  = std::nullopt;
         }

         last_update = steady_clock::now();
         if (promises.size()) {
            try {
               co_await promise::Race(promise::All(std::move(promises)), WaitDone());
            } catch (smc::Timeout const&) {
               std::cerr
                 << "Aircraft[control_loop]: Timeout while sending control updates, skipping update"
                 << std::endl;
            }
         }
      }
...
void
Aircraft::Control::Update(Control const& target) {
   if (target.heading_) {
      if (target.heading_ == Control::CLEAR) {
         heading_.reset();
      } else {
         heading_ = target.heading_;
      }
   }

   if (target.pitch_) {
      if (target.pitch_ == Control::CLEAR) {
         pitch_.reset();
      } else {
         pitch_ = target.pitch_;
      }
   }

   if (target.speed_) {
      if (target.speed_ == Control::CLEAR) {
         speed_.reset();
      } else {
         speed_ = target.speed_;
      }
   }

   if (target.vspeed_) {
      if (target.vspeed_ == Control::CLEAR) {
         vspeed_.reset();
      } else {
         vspeed_ = target.vspeed_;
      }
   }
}
...
Aircraft::TrafficInfoCache::TrafficInfoCache(
  Aircraft&                                  self,
  std::chrono::steady_clock::duration const& refresh_rate
)
   : self_(self)
...
Aircraft::TrafficInfoCache::operator*() {
   return [this] -> Promise<smc::TrafficInfo> {
      auto lock = std::move(co_await promise::LockGuard::Create(mutex_));

      auto const now = std::chrono::steady_clock::now();
      if ((now - last_update_) > REFRESH_RATE) {
         last_update_ = now;
         cached_info_ = co_await self_.main_.SimConnect().GetAircraftInfo(co_await self_.ID);
      }

      auto const result = cached_info_;
      self_.main_.MainPool::Dispatch([lock = std::move(lock)] mutable { lock.Unlock(); }).Detach();
