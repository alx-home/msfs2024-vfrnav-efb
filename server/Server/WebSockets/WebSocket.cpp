...
Server::WebSocket::~WebSocket() { std::cout << "Session " << peer_ << " closed" << std::endl; }
...
Server::WebSocket::OnRead(error_code ec, size_t n) {
   ...
   if (std::holds_alternative<ws::msg::HelloWorld>(message)) {
      ...
      std::cout << std::string_view(hello_world.proto_) << " " << *hello_world.type_ << std::endl;

      if (*hello_world.type_ == "EFB") {
         std::make_shared<EFBWebSocket>(std::move(*this))->Start();
      } else {
