...
SimConnect::RequestFacilityData(std::string_view icao, std::string_view region) {
   ...
   return MakePromise(
      ...
      Ensure([this, request_id] constexpr { pending_facility_.erase(request_id); }).Detach();
   });
}
...
SimConnect::RequestFacilitiesList(SIMCONNECT_FACILITY_LIST_TYPE type) {
   ...
   return MakePromise(
      ...
      Ensure([this, request_id] constexpr { pending_facilities_list_.erase(request_id); }).Detach();
   });
}
