...
template <class FUN>
[[nodiscard]] WPromise<promise::return_t<promise::return_t<FUN>>>
SimConnect::Proxy(FUN&& func) const {
   using RETURN       = promise::return_t<FUN>;
   using PROMISE_TYPE = promise::return_t<RETURN>;

   return [this, func = std::move(func)] mutable -> Promise<PROMISE_TYPE> {
      {
      ...
      try {
         ...
         if constexpr (std::is_void_v<PROMISE_TYPE>) {
            if constexpr (promise::IS_PROMISE_FUNCTION<FUN>) {
               co_await func;
            } else {
               static_assert(
                 promise::IS_WPROMISE<RETURN>,
                 "FUN must return a WPromise if it's not a promise function"
               );
               co_await func();
            }
            co_await main_.MainPool::Dispatch();
             ...
             if (
               std::holds_alternative<
                ...
                std::tie(process, error) = (*process)(
                  std::get<std::reference_wrapper<SIMCONNECT_RECV_FACILITY_DATA const>>(data).get()
                );

                if (!error.empty()) {
                   MakeReject<UnknownError>(*reject, "Failed to process facility data: " + error);
                }
             }
          },
          reject.shared_from_this()
   ...
   return WPromise{
     [this, type, request_id](
       Resolve<std::vector<FacilityType<TYPE>>> const& resolve, Reject const& reject
        ...
        WPromise{[this, reject = reject.shared_from_this(), remaining]() -> Promise<void> {
           co_await Ensure();
