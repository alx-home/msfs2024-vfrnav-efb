...
public:
   ...
   double operator*(Coords const& other) const {
      return [&]<std::size_t... I>(std::index_sequence<I...>) constexpr {
   ...
   Coords operator^(Coords const& other) const {
      return [&]<std::size_t... I>(std::index_sequence<I...>) constexpr {
         return Coords{
           (this->operator[](I) * other.operator[]((I + 1) % N)
            - this->operator[]((I + 1) % N) * other.operator[](I))...
         };
      }(std::make_index_sequence<N>{});
   }
   ...
   template <class TYPE>
      requires(std::is_arithmetic_v<std::remove_cvref_t<TYPE>>)
   Coords& operator*=(TYPE scalar) {
      return [&]<std::size_t... I>(std::index_sequence<I...>) constexpr {
   ...
   template <class TYPE>
      requires(std::is_arithmetic_v<std::remove_cvref_t<TYPE>>)
   Coords Scaled(TYPE scale) const {
      return [&]<std::size_t... I>(std::index_sequence<I...>) constexpr {
   ...
   template <class TYPE>
      requires(std::is_arithmetic_v<std::remove_cvref_t<TYPE>>)
   Coords Clamped(TYPE max_length) const {
      double length = this->Length();
   ...
   template <class TYPE>
      requires(std::is_arithmetic_v<std::remove_cvref_t<TYPE>>)
   Coords Limited(TYPE max_length) const {
      double length = this->Length();
      if (length > max_length) {
         return this->Scaled(max_length / length);
      }
