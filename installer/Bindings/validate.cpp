...
Main::Validate(
  ...
  std::string                           installPath
   ...
   if (lock.result_ == ERROR_ALREADY_EXISTS) {
      co_return Fatal(
        "MSFS VFRNav Server is already running.",
   ...
   if (auto result = win32::CreateLink(
       ...
       !SUCCEEDED(result)) {
      Warning(
        "Couldn't create shortcut (" + std::to_string(result) + "):",
        R"(C:\ProgramData\Microsoft\Windows\Start Menu\Programs\MSFS VFRNav' Server)"
      );
   }
   ...
   if (std::filesystem::exists(communityPath)) {
      if (!std::filesystem::remove_all(communityPath)) {
         co_return Fatal("Couldn't cleanup previous version:", communityPath);
      }
   }

   auto const path = communityPath + "/html_ui/efb_ui/efb_apps/msfs2024-vfrnav/efb";
