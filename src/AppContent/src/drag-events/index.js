...
export function supportsPassiveEventListener() {
...
function updateCentroidCoordinatesOfTouchesIn(coordinateProp, event, outPoint) {
    outPoint.x = event[coordinateProp + "X"];
    outPoint.y = event[coordinateProp + "Y"];
}
...
function createDragEventFromTouch(targetElement, e, type, cancelable, window, dataTransfer, relatedTarget) {
    ...
    dndEvent.screenX = e.screenX;
    dndEvent.screenY = e.screenY;
    dndEvent.clientX = e.clientX;
    dndEvent.clientY = e.clientY;
    dndEvent.pageX = e.pageX;
    dndEvent.pageY = e.pageY;
...
var DragOperationController = (function () {
    function DragOperationController(_initialEvent, _config, _sourceNode, _dragOperationEndedCb) {
        ...
        addDocumentListener("pointermove", this._touchMoveHandler, false);
        addDocumentListener("pointerup", this._touchEndOrCancelHandler, false);
        addDocumentListener("pointercancel", this._touchEndOrCancelHandler, false);
    }
    DragOperationController.prototype._setup = function () {
        ...
        if (!this._dragImageOffset) {
            if (this._config.dragImageOffset) {
                ...
                var cs = getComputedStyle(dragImageSrc);
                this._dragImageOffset = {
                    x: targetRect.left - this._initialEvent.clientX - parseInt(cs.marginLeft, 10) + targetRect.width / 2,
                    y: targetRect.top - this._initialEvent.clientY - parseInt(cs.marginTop, 10) + targetRect.height / 2
                };
            }
        }
    ...
    DragOperationController.prototype._cleanup = function () {
        ...
        removeDocumentListener("pointermove", this._touchMoveHandler);
        removeDocumentListener("pointerend", this._touchEndOrCancelHandler);
        removeDocumentListener("pointercancel", this._touchEndOrCancelHandler);
        if (this._dragImage) {
...
export function polyfill(override) {
    if (override) {
