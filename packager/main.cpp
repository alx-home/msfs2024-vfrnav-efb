      ...
      for (auto const& [name, resource_] : resources) {
         ...
         if (zip) {
            ...
            if (!std::filesystem::exists(resource)) {
               ...
               int const compress_status = compress2(
               ...
               if (compress_status != Z_OK) {
                  throw std::runtime_error(
                    std::format("zlib compression failed for {}: {}", resource__, compress_status)
                  );
               }
...
{0}_END:
      ...
      std::cerr << "packager failed: " << ex.what() << std::endl;
      return 1;
   } catch (...) {
      std::cerr << "packager failed: unknown error" << std::endl;
      return 1;
   }
}
